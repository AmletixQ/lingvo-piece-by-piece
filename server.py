...
class Task:
    ...
    def get_tasks_by_random_id(self):
        ...
        if mode_value == 1:
            session_db = db_session.create_session()
            query_tasks = (
                session_db.query(ModeOne).filter(ModeOne.id_animal == random_id).all()
            )
            session_db.close()
            ...
            session_db = db_session.create_session()
            query_task = session_db.query(ModeTwo).filter(
                ModeTwo.id_animal == random_id
            )
            session_db.close()
    ...
    def get_name_animal(self):
        ...
        random_id = session.get("random_id", 1)
        name_animal = (
            session_db.query(Animal.oset_name).filter(Animal.id == random_id).first()
        )
        session_db.close()
    ...
    def texts_by_level(self):
        ...
        if current_question < len(lst_tasks):
            if mode_value == 1:
                ...
                query = session_db.query(Animal.shape_used).filter(
                    Animal.id == random_id
                )
    ...
    if request.method == "POST":
        ...
        if btn_hint is not None:
            ...
            return render_template(
                "mode_one.html",
                question=question,
                btn_texts=list_button_text_mode1,
                file_imgs=lst_imgs,
                user_points=user_points_mode1,
